...
The dimensions here are raw BOUNDING BOX values, which for most of this model
are wrong: the NX export carries a 0.0416 deg tilt, so a 50.800 face reads
51.095. That is fine for the overlap matching done downstream, but it is why
the real extractor projects vertices onto recovered face normals instead. See
the Dimensions note in extract_stm_geometry.py.

Note the tilt is PIECEWISE, not global -- 208 of 224 solids carry it and 12 are
already square in the CAD (stage 00 prints the split). So the bbox error here is
not uniform: the square solids' boxes are already correct and the tilted ones'
are not. Do not calibrate anything off a fixed 0.3mm assumption; run stage 00
and read its per-solid table.
