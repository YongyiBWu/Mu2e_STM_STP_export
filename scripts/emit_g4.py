...
def _load_materials():
    """solid_id -> Geant4 material, from output/solid_materials.json.

    The iteration-3 STEP names the material in each part label ("248 Lead
    Brick", "Top Al sq walls"), so the material is now KNOWN rather than
    guessed from the CAD colour. That matters: the colour guess was wrong for
    about thirty of the forty-six shapes, and wrong in a way that would pass
    review unnoticed -- "medium steel" alone covers Al6061, BP, Polyethylene
    and MildSteel, and its old guess RackSteel is 98.5% aluminium by mass.
    """
    try:
...
def material_of(shape):
    """The shape's material, or None when the material pass has not run.

    Materials are recorded per solid, so this joins through any one placement.
    Every copy of a shape agrees -- checked: no shape spans two materials.
    """
...
def material_call(shape):
    """findOrBuildMaterial for this shape's material."""
    colour = (shape.get("material") or "?").strip()
    mat = material_of(shape)
    if mat:
        # The colour is kept as provenance only. It is not a material, and
        # naming it as one is what produced the wrong calls this replaces.
        return ('  // %s  (from the CAD part label; CAD colour was "%s")\n'
                '  G4Material* mat = findOrBuildMaterial("%s");'
                % (mat, colour, mat))

    # No material pass: fall back to the old colour guess, clearly marked.
    ...
    return ('  // CAD colour "%s" -> %s (A GUESS -- run the material pass)\n'
            '  G4Material* mat = findOrBuildMaterial("%s");'
            % (colour, hint, hint))
...
def main():
    ...
    L.append("// Geometry is derived. Materials and names come from the CAD")
    L.append("// part labels (output/solid_materials.json); the CAD colour is")
    L.append("// recorded beside each as provenance, not as a material.")
